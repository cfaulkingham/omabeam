...
def package(binary, target, licenses, output, root=ROOT, encoder_helper=None, cast_helper=None, cast_licenses=None):
    ...
    if (cast_helper is None) != (cast_licenses is None):
        raise ValueError("Cast helper and its license inventory must be provided together")
    if cast_helper is not None:
        validate_binary(cast_helper, target)
    ...
    for path in sorted((root / "docs/qa").glob("*.md")):
        files[str(path.relative_to(root))] = path
    ...
    if cast_helper is not None:
        files["omarchy-plugin/native/bin/omabeam-cast"] = cast_helper
        manifest_path = cast_licenses / "manifest.json"
        cast_manifest = json.loads(manifest_path.read_text())
        records = cast_manifest.get("files", [])
        if not records or not cast_manifest.get("upstream"):
            raise ValueError("Cast license inventory is empty")
        binary_record = cast_manifest.get("binary", {})
        if binary_record.get("protocol") != 1 or binary_record.get("sha256") != hashlib.sha256(cast_helper.read_bytes()).hexdigest():
            raise ValueError("Cast helper does not match its build/license inventory")
        files["licenses/cast/manifest.json"] = manifest_path
        for record in records:
            name = record["path"]
            if not name or Path(name).is_absolute() or ".." in Path(name).parts:
                raise ValueError("Unsafe Cast notice path")
            path = cast_licenses / name
            if not path.resolve().is_relative_to(cast_licenses.resolve()) or path.is_symlink():
                raise ValueError("Unsafe Cast notice symlink")
            if hashlib.sha256(path.read_bytes()).hexdigest() != record["sha256"]:
                raise ValueError("Cast notice checksum mismatch")
            files[f"licenses/cast/{name}"] = path
            ...
            with gzip.GzipFile(filename="", fileobj=temporary, mode="wb", mtime=0) as compressed:
                with tarfile.open(fileobj=compressed, mode="w") as tar:
                    for name, path in sorted(files.items()):
                        ...
                        info.mode = 0o755 if name in ("install.sh", "omarchy-plugin/omabeam", "omarchy-plugin/native/bin/omabeam", "omarchy-plugin/native/bin/omabeam-encoder", "omarchy-plugin/native/bin/omabeam-cast") else 0o644
...
def main():
    ...
    parser.add_argument("--cast-helper", type=Path)
    parser.add_argument("--cast-licenses", type=Path, help="Notice directory generated by build-cast.py")
    ...
    try:
        print(package(args.binary, args.target, args.licenses, args.output, encoder_helper=args.encoder_helper,
                      cast_helper=args.cast_helper, cast_licenses=args.cast_licenses))
