...
def package(binary, target, licenses, output, root=ROOT):
            ...
            with gzip.GzipFile(filename="", fileobj=temporary, mode="wb", mtime=0) as compressed:
                with tarfile.open(fileobj=compressed, mode="w") as tar:
                    for name, path in sorted(files.items()):
                        ...
                        info.mode = 0o755 if name in ("install.sh", "omarchy-plugin/omabeam", "omarchy-plugin/native/bin/omabeam") else 0o644
...
def main():
    ...
    try:
        print(package(args.binary, args.target, args.licenses, args.output))
